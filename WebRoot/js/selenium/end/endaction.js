 $("#steptitle").text("end 脚本");
 $(function(){
	findTestStepList();
});
//----------------------------获取action中所有的用例集--------------------------------------------------
function findTestStepList(){
  $.getJSON('../../selenium/findNoActionTestSteps.do',{'stage':'end'},
			function(data) {
			...
			if(data!=null&&data.length!=0){
				$.each(data,function(i,n){
					...
					$(temp).children("td:eq(6)").children().val(n.snapshot);
					$("#initSteplist").append(temp);
				});
			}
			$("#initSteplist").append("<tr>"+lastTr+"</tr>");
			});
  }
...
function addSteps(){
	...
	$("tr:gt(0)").each(function(i,n){
		...
		step.snapshot = $(n).find("td:eq(6)").children().val();
	...
	if(flag == "0"){ return;}
	$.getJSON('../../selenium/saveNoActionSteps.do',{'steps':JSON.stringify(steps),'stage':'end'},
			function(data) {
				if(data==0){
						alert("保存失败！");
					 }else{
						 alert("保存成功！");
							//$(".panel-body").htmlLoad("page/selenium/init/.jsp");
						}
			});
}